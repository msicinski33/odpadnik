...
const ImportModal = ({ open, onClose, title, requiredColumns, templateFileName, onImport, entityType, description }) => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl relative animate-fade-in">
          <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-t-xl px-6 py-4 flex items-center justify-between">
            <h2 className="text-white text-xl font-bold">Potwierdź import danych</h2>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md relative animate-fade-in">
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-t-xl px-6 py-4 flex items-center justify-between">
          <h2 className="text-white text-xl font-bold">{title || `Zaimportuj dane pracowników`}</h2>
          <button onClick={onClose} className="text-white hover:text-blue-200">
            <X className="h-6 w-6" />
          </button>
        </div>
        <CardContent className="p-6">
          <p className="text-gray-700 mb-4 text-center">{description || `Prześlij plik Excel z informacjami o pracownikach`}</p>
